...
- `mqtt_allow_anonymous`
  - 預設 `false`，不允許匿名連線。
  - 只有明確設為 `true` 才會允許所有匿名連線，且忽略 `mqtt_username` 與 `mqtt_password`。
- `mqtt_username`
  - 本地 broker 的連線帳號；匿名連線關閉時不可為空白。
- `mqtt_password`
  - 本地 broker 的連線密碼；匿名連線關閉時不可為空白。

預設驗證成功的使用者可讀寫所有 topic。若匿名連線關閉但帳號或密碼未設定，broker 會拒絕啟動。若 `mqtt_tls_cert` 或 `mqtt_tls_key` 未設定，則只會啟用 `tcp/ws`，`ssl/wss` 會被略過。
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/MarsClient"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/MarsJSON"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/MQTTClient"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/MQTTServer"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/HttpService"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/Security"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/Tools"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/DataCompress"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/AsyncTaskProcessor"
```
...
```go
import "github.com/MarsSemi/MarsCloud-SaaS/SDK/ScriptExecutor"
```
